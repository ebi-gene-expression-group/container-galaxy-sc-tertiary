...
parser.add_argument('--cell_prefix',
                    dest='cell_prefix',
                    default='',
                    help='Prefix to apply to cell barcodes')
...
cb_names = [
            cell_prefix
            + s for s in pd.read_csv(cb_file, header=None)[0].values
            ]
...
if len(cb_names) != nrows:
    print("The number of matrix rows (%d) does not match the number of "
          "supplied barcodes (%d)" % (nrows, len(cb_names)))
    sys.exit(1)

if len(gene_names) != ncols:
    print("The number of matrix columns (%d) does not match the number of "
          "supplied genes (%d)" % (ncols, len(gene_names)))
    sys.exit(1)
...
genes_frame.to_csv(path_or_buf='%s/genes.tsv' % mtx_out, index=False,
                   sep="\t", header=False)
